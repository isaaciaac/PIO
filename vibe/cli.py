...
def chat(
    ...
    if client == "vscode" or vscode_env:
        exec_hint = (
            ...
            "注意：你不能假设系统一定会自动触发工作流；你只能给出清晰的下一步与必要信息。"
            "禁止说「我将立即创建/我已经开始执行」这类承诺式话术；请用条件句："
            "「如果进入写项目流程/如果系统开始执行，我会…；否则我先基于现状给出定位与方案」。"
        )
    else:
        exec_hint = (
    ...
    system = (
        ...
        f"{mode_banner}\n"
        "重要：你现在只是在“对话”模式下回答问题。不要声称或承诺“已经/将要 生成/创建/运行/修改”。\n"
