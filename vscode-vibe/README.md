...
- 用下拉框选择权限模式（只影响是否允许本地工具动作/是否逐项确认）：
  ...
  - `完全授权（不询问）`：不弹窗，全部允许
- 当权限为 `仅聊天（禁用工具）` 时，可以选择“角色”（PM/架构/安全/工程等）进行对话；写项目模式不会在这里继续对话，而是直接跑工作流（PM/Router/Coder/QA）
...
- `确认权限/完全授权`：若未初始化则先 `vibe init`，然后直接执行工作流：
  1. `vibe task add "<你的输入>"`
  2. `vibe run --task <task_id> --route <auto/L0..L4>`（Mock 勾选时会追加 `--mock`）

如果你想把“聊天模式里多轮累积的草稿”一次性执行：切到 `确认权限/完全授权` 后发送 `执行`（或 `/run`）。
