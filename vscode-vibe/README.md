...
- 用下拉框选择权限模式（只影响是否允许本地工具动作/是否逐项确认）：
  ...
  - `完全授权（不询问）`：不弹窗，全部允许
- 当权限为 `仅聊天（禁用工具）` 时，可以选择“角色”（PM/架构/安全/工程等）进行对话；写项目模式默认仍由 PM 梳理需求，信息足够会自动触发工作流（Router/PM/Coder/QA）落地到代码
- 可选择“路由等级”（`auto/L0..L4`），会在运行时传给 `vibe run --route ...`
- 可选择“风格”（`free/balanced/detailed`），会作为 `--style` 传给 `vibe chat` / `vibe run`
- 勾选 `Mock` 可无 key 运行闭环
- 点击 `发送`（或 `Ctrl+Enter` 发送）

每次发送的行为取决于模式：

- `仅聊天（禁用工具）`：若未初始化则先 `vibe init`，然后执行 `vibe chat "<你的输入>" --json`（Mock 勾选时会追加 `--mock`）
- `确认权限/完全授权`：若未初始化则先 `vibe init`，然后根据你的输入自动决策：
  - 如果看起来已经是可执行需求：直接执行工作流（`vibe task add ...` → `vibe run ...`）
  - 如果信息不足：先 `vibe chat ...` 追问补齐；当信息足够时自动执行工作流并落地到代码
